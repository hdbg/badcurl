...
impl Error {
    ...
    pub fn description(&self) -> &str {
        unsafe {
            let s = badcurl_sys::curl_easy_strerror(self.code);
            assert!(!s.is_null());
...
impl ShareError {
    ...
    pub fn description(&self) -> &str {
        unsafe {
            let s = badcurl_sys::curl_share_strerror(self.code);
            assert!(!s.is_null());
            str::from_utf8(CStr::from_ptr(s).to_bytes()).unwrap()
        }
    }
...
impl MultiError {
    ...
    pub fn description(&self) -> &str {
        unsafe {
            let s = badcurl_sys::curl_multi_strerror(self.code);
            assert!(!s.is_null());
            str::from_utf8(CStr::from_ptr(s).to_bytes()).unwrap()
        }
    }
...
impl FormError {
    ...
    pub fn description(&self) -> &str {
        match self.code {
            badcurl_sys::CURL_FORMADD_MEMORY => "allocation failure",
            badcurl_sys::CURL_FORMADD_OPTION_TWICE => "one option passed twice",
            badcurl_sys::CURL_FORMADD_NULL => "null pointer given for string",
            badcurl_sys::CURL_FORMADD_UNKNOWN_OPTION => "unknown option",
            badcurl_sys::CURL_FORMADD_INCOMPLETE => "form information not complete",
            badcurl_sys::CURL_FORMADD_ILLEGAL_ARRAY => "illegal array in option",
            badcurl_sys::CURL_FORMADD_DISABLED => {
...
impl From<ffi::NulError> for Error {
    fn from(_: ffi::NulError) -> Error {
        Error {
            code: badcurl_sys::CURLE_CONV_FAILED,
            extra: None,
        }
    }
}
